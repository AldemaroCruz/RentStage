...
ASSISTANT_AI_MODE="${ASSISTANT_AI_MODE:-rules}"
ASSISTANT_AI_LOCATION="${ASSISTANT_AI_LOCATION:-$GCP_REGION}"
ASSISTANT_AI_MODEL="${ASSISTANT_AI_MODEL:-gemini-2.5-flash}"
ASSISTANT_AI_TIMEOUT="${ASSISTANT_AI_TIMEOUT:-20s}"
ALLOW_DEMO="false"
[[ "$SEED_DEMO_DATA" == "true" ]] && ALLOW_DEMO="true"
case "$ASSISTANT_AI_MODE" in
  rules|vertex) ;;
  *)
    echo "ASSISTANT_AI_MODE must be rules or vertex" >&2
    exit 1
    ;;
esac
...
gcloud run deploy "$API_SERVICE" \
  ...
  --set-env-vars "APP_ENV=staging,FIREBASE_PROJECT_ID=${GCP_PROJECT_ID},GCLOUD_PROJECT=${GCP_PROJECT_ID},SEED_DEMO_DATA=${SEED_DEMO_DATA},ALLOW_DEMO_DATA_OUTSIDE_LOCAL=${ALLOW_DEMO},SESSION_DURATION=12h,SESSION_COOKIE_NAME=rentstage_session,TENANT_COOKIE_NAME=rentstage_tenant,CSRF_COOKIE_NAME=rentstage_csrf,COOKIE_SECURE=true,REQUIRE_VERIFIED_EMAIL=false,LOCAL_AUTH_BOOTSTRAP=false,WEB_BASE_URL=${PLACEHOLDER_ORIGIN},CORS_ALLOWED_ORIGINS=${PLACEHOLDER_ORIGIN},ASSISTANT_AI_MODE=${ASSISTANT_AI_MODE},ASSISTANT_AI_PROJECT_ID=${GCP_PROJECT_ID},ASSISTANT_AI_LOCATION=${ASSISTANT_AI_LOCATION},ASSISTANT_AI_MODEL=${ASSISTANT_AI_MODEL},ASSISTANT_AI_TIMEOUT=${ASSISTANT_AI_TIMEOUT},ASSISTANT_AI_MAX_OUTPUT_TOKENS=512" \
  --quiet
